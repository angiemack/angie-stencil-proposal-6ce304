...
3. **Wire the CTAs to the real auth routes.** Primary CTA → `/signup`; secondary / "Sign in" → `/login`. These exist via `stencilAuthRoutes` in `routes.ts` — use `<Link to="/signup">` / `<Link to="/login">`.
